...
class ScreenHome extends StatelessWidget {
  ...
  builder: (context, state) {
    return state.isLoading==true?Center(child: CircularProgressIndicator()):  ValueListenableBuilder(valueListenable: Scoll, builder: (BuildContext context, index, _) {
          return NotificationListener<UserScrollNotification>(

            onNotification: (notification){
              final ScrollDirection direction=notification.direction;
              if (direction==ScrollDirection.reverse){
                Scoll.value =false;}else if(direction==ScrollDirection.forward){
                Scoll.value =true;
              }
            ...
            child: Stack(
              children: [
                ListView.builder(itemCount: 1,shrinkWrap: true,
                itemBuilder: (BuildContext context, int index) {    return Column(
                  children:  <Widget>[
                    StackPic(index: index,),
                    kh5,
               ...
               Scoll.value == true? AnimatedContainer(
                 duration: const Duration(microseconds: 1000),
                 height: 80,width: double.infinity,color:backgroundColor.withOpacity(0.3),child: Column(
                 children: [
                   Row(

                     children: [ kw10,
Image. network("https://i.pinimg.com/originals/8c/74/0b/8c740bc13bd5a0a19c24d28dff98cbdd.png",width: 50,height: 50,),
                     const Spacer(),
